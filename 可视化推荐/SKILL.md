---
name: 可视化推荐
description: 从 19 种标准学术图表类型中，为实验结果推荐最合适的 1-2 种可视化方案，适用于顶会论文。
version: 1.0.0
author: Shang Zongjiang
license: MIT
tags: [可视化, 实验结果, 图表, 学术写作, 数据分析, NeurIPS, CVPR]
---

# 实验绘图类型推荐

根据实验数据和结论目标，从 19 种标准学术图表中推荐最佳可视化方案，提供完整的视觉设计规范。
...
**四、数据关系与矩阵可视化类**
11. **热力图**：特别适用于呈现大规模的矩阵形式数据。通过颜色深浅直观反映数值大小，常用于展示混淆矩阵、多模型多任务性能对比矩阵或特征相关性矩阵。
...
3. **视觉设计规范**：
   ...
   - 尺度处理：若涉及数据差异巨大，给出断裂轴、对数坐标或归一化的具体建议。
   - 统计要素：若适用，说明误差线、拟合曲线或显著性标记的要求。
   - 配色与样式：提供具体的配色策略及线型建议。

如未收到输入，直接询问：「请粘贴实验数据（推荐直接复制 Excel/CSV 表格）并简述你想强调的核心结论」
